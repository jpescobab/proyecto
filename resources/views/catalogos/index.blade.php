    ...
    <div id="content" class="main-content">
        <div class="layout-px-spacing">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div>
                    <h4>Listado Catalogos Presupuestarios</h4>
                </div>

                <div>
                    <!-- Contenido derecho -->
                    <a href="{{ route('catalogos.create') }}" class="btn btn-outline-primary btn-sm">
        ...
        <tbody>
            @foreach ($catalogos as $catalogo)
                <tr>
                    ...
                    <td>{{ Str::limit($catalogo->nombre, 60) }}</td>
                    <td>{{ Str::limit($catalogo->descripcion, 100) }}</td>                
                    <td>{{ $catalogo->estado }}</td>
                    <td>{{ $catalogo->item->nombre ?? 'No asignado' }}</td>
                    <td class="text-center">
                        <a href="{{ route('catalogos.show', $catalogo) }}" class="btn btn-sm btn-outline-primary" title="Ver">
                            <i class="fas fa-eye"></i>
                        </a>
                        {{-- <a href="{{ route('catalogos.edit', $catalogo) }}" class="btn btn-sm btn-outline-warning" title="Editar">
                            <i class="fas fa-edit"></i>
                        </a> --}}
                        {{-- <form action="{{ route('catalogos.destroy', $catalogo) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Está seguro de eliminar este catálogo?');">
                            @csrf
                            @method('DELETE')
